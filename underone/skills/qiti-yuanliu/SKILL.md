...
> **世界观**：炁是异人异能的本质能量流。对话上下文是 Agent 的"炁脉"——炁场纯净（entropy 低）则思路清晰；炁体污浊（矛盾/冗余/漂移）则需诊断修复。
> - **炁熵**：上下文能量的无序度，数值越高说明炁场越混乱
> - **炁印**（`dna_snapshot`）：决策能量指纹，追溯本源炁体特征
> - **炁归契约**（`repair_handoff`）：炁体无法自愈时，移交大罗洞观执行深度追踪
> - **炁层次**：`excellent`(先天炁，纯净) → `good`(后天炁，正常) → `warning/danger`(后天炁受损)
>
> **异人语录**（张怀义）：
> - 天人合一：「炁体源流，万法归一」——先天炁充盈，上下文如明镜止水
> - 先天炁成：「我这一辈子，就做了这一件事」——炁脉尚稳，继续前行
> - 后天炁聚：「炁体污浊，需归本源」——上下文出现裂痕，需诊断修复
> - 炁脉初开：「这股力量……不属于我」——炁场崩塌，立即执行稳态修复
...
**职责边界协议**：炁体源流是十技中的 `observer`。它负责诊断、稳态契约、规则候选与 handoff 建议；它**不负责**直接改写其他 skill、持久化全局阈值，发现复杂矛盾时应移交 `dalu-dongguan` 或 `xiushen-lu`。
