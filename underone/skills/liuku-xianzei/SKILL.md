...
>
> **V5.5** — 世界观深度注入：炁化吸收 · 尸魔侵蚀 · 炁机循环隐喻体系

> **世界观**：六库仙贼以窃取为手段，将外部信息萃取后分藏于六座内在信息库。"仙贼"之道：取的都是有价值的东西，经炼化后化为自身宝藏。巴伦的六库仙贼被尸魔污染后无法使用，本模块将"污染风险"类比为"尸魔侵蚀"。
> - **六库**：法律法规库（7天）/ 技术方案库（90天）/ 行业数据库（180天）/ 社区经验库（90天）/ 基础理论库（3年）/ 默认库（180天）
> - **炁化归元**（`inheritance_queue`）：经炼化品质合格，炁化完成归入长期记忆
> - **尸魔封印**（`quarantine_queue`）：疑似尸魔侵蚀，放封印区等待净化复核
> - **尸魔侵蚀**（`contamination_risk`）：混入六库的尸魔污染，需防止扩散腐蚀本库
> - **炁机循环**（`review_schedule`）：炁不流转则枯，需按艾宾浩斯节律定期炁化巩固
> - **信息腐蚀**（`information_erosion`，V5.6）：呼应"口水附炁腐蚀万物"——低质/被污染的信息如口水般持续腐蚀本库炁机，需评估腐蚀压力并防扩散
> - **无痕消化**（`trace_free_digestion`，V5.6）：呼应"气息与天地同化、几乎消失"——优质洁净的知识被平滑吸收，对既有炁脉的扰动趋近于零
> - **全格式吞噬**（`devour_any` / CLI `--devour`，V5.7）：呼应"什么都敢吃、吃了就能用"——PDF / 音视频转录 / 代码仓库 / 聊天记录 / 非结构化噪音皆为"食物"，统一归一并标注 `source_format` 后炼化
> - **精华输出**（`essence_units`，V5.7）：消化后直接吐出**可复用的知识单元**（要点/可执行/出处），而非只给"消化率73%"
> - **保鲜激活**（`activation_index`，V5.7）：呼应"保鲜"落地——消化的知识按触发词在后续对话**自动激活复现**，而非存进向量库吃灰（区分 live / dormant）
>
> **悟得者**：刘雨石（六库仙贼真正悟得者）｜ **语录人物**：巴伦（盗术传人）
>
> **异人语录**（巴伦）：
> - 六库全开：「六库尽开，万物皆可吞」——消化率极高，知识已内化
> - 三库初通：「消化尚可，还需时日」——部分吸收，待巩固
> - 初尝百味：「此物难消，先存后议」——消化率低，需反刍复习
> - 尸魔封印：「此物有毒，先行隔离」——可疑信息已隔离
> - 尸魔侵蚀：「知识污染，小心为上」——检测到污染风险
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 5.5 | 当前 | **世界观深度注入**：digestion_lore(炁化完成/炼化中/未入炁)、contamination_lore(尸魔侵蚀/炁机浑浊/炁机清朗)、inheritance_queue→炁化归元、quarantine_queue→尸魔封印、review_schedule→炁机循环；CLI输出隐喻化；recommendation文案世界观对齐 |
