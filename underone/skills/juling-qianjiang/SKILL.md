...
> **世界观**：拘灵遣将是驭使鬼魂灵体的异能——拘押野鬼训练成兵，按阵型派遣执行任务，最高境界是"百鬼夜行"。
> - **灵体**（`spirits`）：工具/子代理，被拘押后可遣派
> - **阵型三式**：孤魂镇场（`single-possession`）/ 将帅双星（`dual-attunement`）/ 百鬼夜行（`night-parade`）
> - **降级双策**：护符（`protect`，守柔降级保护）/ 强制附体（`possess`，以灵体意志强行完成）
> - **反叛风险**（`rebellion_risk`）：灵体失控评估，low/medium/high 三级预警
> - **灵魂契约**（`invocation_protocol`）：从 soul.md 提取的调用规约，约束灵体行为边界
> - **天条**（`avoid_constraints`）：灵体本身不得触犯的禁忌，违则自毁
>
> **异人语录**（风正豪）：
> - 孤魂镇场：「一灵镇场，独当一面」——单兵作战模式
> - 将帅双星：「将帅双星，攻守兼备」——主副灵协作模式
> - 百鬼夜行：「百鬼夜行，万灵听令」——全阵出击模式
> - 灵体反叛：「灵体反叛！加强拘束」——检测到反叛风险
> - 主灵力竭：「主灵力竭，副灵接阵」——执行降级切换
