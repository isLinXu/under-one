...
> **世界观**：通天箓是符箓之术的极致——每一个任务意图都可以被凝炼成一枚枚符箓，按阵型排布、相生相克。本质是顶级"言出法随"，废除一切准备工序、徒手凭空成符。
> - **符箓**：任务的最小执行单元，六种类型各司其职（查阅/推演/创生/化形/印鉴/断断）
> - **符阵**（`topology`）：多枚符箓按拓扑排列，构成完整的任务阵法
> - **禁咒**（`curse_level`）：高危任务若处理不当，施术者本身可能遭到反噬
> - **编排模式**：速符（`quick-cast`）/ 中阵（`balanced-array`）/ 大阵法（`full-ritual`）
> - **言出法随**（`speak()` / CLI `--speak`，V5.9）：传统符箓需设坛行炁、备料数时至数日（对应结构化 spec 的前置工序），言出法随废除一切准备，一句自然语言即刻成符
> - **符种细分**（`species_catalog`，V5.9）：呼应漫画符种极其丰富——六类功能符箓按符种家族归类（侦查/攻击/防御/化形/加速型，另保留封印/治疗型），符阵招式构成一目了然
> - **即时画符**（`instant_fu`，V6.0）：呼应陆瑾/郑子布"不需任何前置仪式就能凭空画符"——看到任务即从动作动词/对象**零 shot 即兴合成一道可直接执行(executable)的符**，哪怕没有任何预定义维度命中，产出可直接扔给 executor
> - **符箓叠加**（`fu_stack`，V6.0）：呼应符箓可无限叠加组合——诸符按可并行层叠加组合（而非线性 A→B→C），威力相生
>
> **悟得者**：郑子布 ｜ **语录人物**：郑子布（传承者陆瑾 → 张灵玉）
>
> **异人语录**（郑子布）：
> - 速符起手：「符到即行，不拘形式」——瞬发符箓，快速拆解
> - 中阵编排：「列阵通天，符箓连环」——均衡布阵，步步为营
> - 大阵法：「通天大阵，六箓齐发」——完整仪式，全链执行
> - 禁咒反噬：「此箓有反噬之险」——检测到高风险，需谨慎施法
